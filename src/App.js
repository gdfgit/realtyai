  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('|') && line.endsWith('|')) {
      if (!inTable) {
        inTable = true;
        headerDone = false;
        tableHTML = '<table style="width:100%;border-collapse:collapse;margin:12px 0;font-size:13px;border-radius:8px;overflow:hidden;">';
      }
...
function buildPropertyResponse(query, searchResults) {
  ...
  // Property Photos - display as actual images
  if (images.length > 0) {
  ...
  // Listings - clean content, only show title + short desc + View Full Listing link
  results.forEach((r, i) => {
    output += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    output += `**${r.title}**\n\n`;
    output += `${cleanContent(r.content)}\n\n`;
    output += `🔗 [View Full Listing](${r.url})\n\n`;
  });

  // Use location from query for area links (much more reliable than parsing titles)
  const areaAddress = location || extractAddress(results[0]?.title || "") || "property location";
  const encodedAddr = encodeURIComponent(areaAddress);
  ...
  if (!isCommercial && results.length > 0) {
    ...
    // Local Area Links - use the actual location from the query
    output += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    output += `📍 **Explore the Area** — ${areaAddress}\n\n`;
